...
@onready var interactable_component_2: InteractableComponent = $InteractableComponent2
...
func _ready() -> void:
	interactable_component_2.interactable_activated.connect(on_interactable_activated)
	interactable_component_2.interactable_deactivated.connect(on_interactable_deactivated)
	interactable_label_component.hide()
	
	GameDialogueManager.feed_animals.connect(on_feed_animals)
	feed_component.food_recieved.connect(on_food_recieved)
...
func on_feed_animals()->void:
	if in_range:
		trigger_feed_animals("corn",corn_harvest_scene)
		trigger_feed_animals("tomato",tomato_harvest_scene)

func trigger_feed_animals(inventory_item:String,scene:Resource)->void:
	...
	for index in inventory_item_count:
		...
		var time_delay=randf_range(0.1,0.2)
		await get_tree().create_timer(time_delay).timeout
		
		var tween=get_tree().create_tween()
		tween.tween_property(harvest_instance,"position",target_position,0.3)
		tween.tween_property(harvest_instance,"scale",Vector2(0.5,0.5),0.3)
